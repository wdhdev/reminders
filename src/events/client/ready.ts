    ...
    async execute(client: ExtendedClient) {
        try {
            ...
            async function manageExistingReminders() {
                ...
                for (const reminder of dueReminders) {
                    ...
                    const recurring = new EmbedBuilder()
                        .setColor(client.config.embeds.default as ColorResolvable)
                        .setDescription(
                            "This reminder was a recurring reminder, however since it was sent overdue, it has been reset, you will need to set it again if you want it to continue."
                        );
                    ...
                    if (reminder?.send_in_channel && reminder.channel) {
                        try {
                            ...
                            await channel.send({
                                ...
                                embeds: reminder?.recurring ? [embed, recurring] : [embed]
                            });
                        } catch {
                            try {
            ...
            manageExistingReminders().then(async () => {
                setInterval(async () => {
                    const reminders = (await Reminder.find({})).filter((reminder) =>
                        client.channels.cache.has(reminder.channel)
                    );
